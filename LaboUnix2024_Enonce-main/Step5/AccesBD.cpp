...
void envoyerRequeteAchatBD(MESSAGE& m)
{
  int pidCaddie = m.expediteur;
  int idArticle = m.data1;
  ...
  if ((ResultSet = mysql_store_result(connexion)) == NULL) // Si la base de donnees n'a pas envoye de resultat
  {
    fprintf(stderr,"(ACCESBD %d) (ERROR)reception requete bd echoue\n",getpid());
    return;
  }
  ...
  if ((resultat = mysql_fetch_row(ResultSet)) != NULL)
  {
    ...
    // Verifier s'il y a assez de stock
    if (atoi(resultat[3]) - quantite >= 0)
    {
      sprintf(requete, "UPDATE UNIX_FINAL SET stock = stock - %d WHERE id = %d;", quantite, idArticle);
...
void envoyerMessage(MESSAGE& m)
{
  switch (m.requete)
  {
    case CONSULT:
      ...
      break;
    case ACHAT:
      fprintf(stderr, "(ACCESBD %d) (PROCESS) Envoi d'une reponse ACHAT a %d : --%d--\n", pidAccesBD, m.type, m.data1);
